  ...
  try {
    ...
    const response = await openai.chat.completions.create({
      ...
      messages: [
        { 
          role: "system", 
          content: "You are an expert football coaching analyst. Provide comprehensive, detailed analysis for ALL requested sections without leaving anything empty. Always respond with valid JSON format." 
        },
        { role: "user", content: enhancedPrompt }
      ],
      max_tokens: 4000,
      temperature: 0.2,
      timeout: 25000 // 25 seconds for deployment compatibility
    });
      ...
      try {
        const enhancementResponse = await openai.chat.completions.create({
          ...
          messages: [
            { 
              role: "system", 
              content: "Fill in missing analysis sections with detailed, authentic content based on the transcript. Always respond with valid JSON format." 
            },
            { role: "user", content: enhancementPrompt }
          ],
          max_tokens: 2000,
          temperature: 0.2,
          timeout: 20000 // 20 seconds for deployment
        });
