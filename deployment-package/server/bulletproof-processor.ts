...
export async function bulletproofProcessVideo(videoId: number): Promise<ProcessingResult> {
  ...
  try {
    ...
    // Step 5: Perform comprehensive multi-AI analysis
    ...
    try {
      ...
      // Perform multi-AI analysis with validated parameters
      multiAIResult = await performMultiAIAnalysis(
        params.transcript,
        params.duration,
        params.videoId,
        params.sessionType,
      ...
      console.error(`[BULLETPROOF] Multi-AI analysis failed, falling back to basic analysis:`, error);
      
      // Fallback to basic OpenAI analysis
      const basicAnalysis = await analyzeWithFallback(
        transcriptionResult.transcript!,
        video,
        transcriptionResult.duration
      );
      ...
      if (!basicAnalysis.success) {
        console.error(`[BULLETPROOF] All analysis methods failed for video ${videoId}:`, basicAnalysis.error);
        await storage.updateVideoStatus(videoId, 'failed', 0);
        return basicAnalysis;
      }
      
      // Create a simplified multi-AI result for compatibility
      multiAIResult = {
        openaiAnalysis: basicAnalysis.analysis,
        claudeAnalysis: null,
        perplexityAnalysis: null,
        synthesizedInsights: {
          overallScore: basicAnalysis.analysis?.overallScore || 75,
          keyStrengths: ['OpenAI analysis completed'],
          priorityDevelopmentAreas: ['Enhanced AI analysis unavailable'],
          researchBackedRecommendations: ['Basic analysis provided'],
          pedagogicalInsights: ['Limited to OpenAI insights'],
          professionalBenchmarking: 'Basic analysis completed'
        }
      };
    }
    ...
    try {
      ...
      console.log(`📋 Quality metrics:`, {
        sectionsCompleted: completenessResult.qualityMetrics.sectionsCompleted,
        totalSections: completenessResult.qualityMetrics.totalSections,
        missingFields: completenessResult.qualityMetrics.missingCriticalFields
      });
      
      if (completenessResult.identifiedGaps.length > 0) {
        console.log(`🔧 Fixed ${completenessResult.identifiedGaps.length} analysis gaps:`);
        completenessResult.identifiedGaps.forEach(gap => {
          console.log(`   - ${gap.section}: ${gap.missingFields.join(', ')} (cause: ${gap.cause})`);
        });
      }
      ...
      multiAIResult = qaEnhancedResult;
