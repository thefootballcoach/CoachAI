import { 
  ...
  developmentPlans, developmentGoals, developmentActions, errorLogs, coachDiaryEntries,
  ...
  type CoachDiaryEntry, type InsertCoachDiaryEntry
} from "@shared/schema";
...
export interface IStorage {
  ...
  // Coach Diary
  getCoachDiaryEntries(coachId: number): Promise<any[]>;
  getCoachDiaryEntry(id: number): Promise<any>;
  createCoachDiaryEntry(data: any): Promise<any>;
  updateCoachDiaryEntry(id: number, data: any): Promise<any>;
  deleteCoachDiaryEntry(id: number): Promise<void>;
  getUpcomingDiaryEntries(coachId: number): Promise<any[]>;
  markDiaryEntryCompleted(id: number): Promise<any>;
...
export class DatabaseStorage implements IStorage {
  ...
  // Coach Diary Methods
  async getCoachDiaryEntries(coachId: number): Promise<CoachDiaryEntry[]> {
    return await db
      .select()
      .from(coachDiaryEntries)
      .where(eq(coachDiaryEntries.coachId, coachId))
      .orderBy(desc(coachDiaryEntries.scheduledDate));
  }

  async getCoachDiaryEntry(id: number): Promise<CoachDiaryEntry | undefined> {
    const [entry] = await db
      .select()
      .from(coachDiaryEntries)
      .where(eq(coachDiaryEntries.id, id));
    return entry;
  }

  async createCoachDiaryEntry(data: InsertCoachDiaryEntry): Promise<CoachDiaryEntry> {
    const [entry] = await db
      .insert(coachDiaryEntries)
      .values({
        ...data,
        createdAt: new Date(),
        updatedAt: new Date()
  ...
  async updateCoachDiaryEntry(id: number, data: Partial<CoachDiaryEntry>): Promise<CoachDiaryEntry> {
    const [entry] = await db
      .update(coachDiaryEntries)
      .set({
        ...data,
        updatedAt: new Date()
      })
  ...
  async deleteCoachDiaryEntry(id: number): Promise<void> {
    await db.delete(coachDiaryEntries).where(eq(coachDiaryEntries.id, id));
  }

  async getUpcomingDiaryEntries(coachId: number): Promise<CoachDiaryEntry[]> {
    const now = new Date();
    ...
    return await db
      ...
      .where(
        and(
          eq(coachDiaryEntries.coachId, coachId),
          eq(coachDiaryEntries.status, 'pending'),
          gte(coachDiaryEntries.scheduledDate, now),
          lte(coachDiaryEntries.scheduledDate, oneWeekFromNow)
        )
      )
      .orderBy(asc(coachDiaryEntries.scheduledDate));
  }

  async markDiaryEntryCompleted(id: number): Promise<CoachDiaryEntry> {
    const [entry] = await db
      .update(coachDiaryEntries)
      .set({
        status: 'completed',
